...
link createBinaryTree(char* s, int* initial, int* final){ 	//constructs binary tree

	while(!(s[*final] == 'r' || s[*final] == 'b' || s[*final] == '.' || s[*final] == '\0')){
		(*final)++;
	}
	if(s[*final] == '\0'){
		return z;
	...
	if(s[*final] == '.'){
		...
		*initial = *final;
		...
		int color;
		char* tempvalue = (char*)malloc(((*final)-(*initial))*sizeof(char));
	  memcpy(tempvalue, &s[*initial], *final);
	  tempvalue[(*final)-(*initial)] = '\0'; //gets string of %d%c
		if(s[*final] == 'r'){
			color = 1;
		}else{
			color = 0;
    ...
    *initial = *final;
    ...
    // link test = NEW(atoi(tempvalue),createBinaryTree(s, initial, final),createBinaryTree(s, initial, final),0);
		link test = (link)malloc(sizeof( *test));
		test->item = atoi(tempvalue);
		test->red = color;
		test->N = 0;
		test->l = createBinaryTree(s, initial, final);
		test->r = createBinaryTree(s, initial, final);
...
void STdeserialize(char* s){
  ...
  int final = 0;
	int color;
  while(!(s[final] == 'r' || s[final] == 'b' || s[final] == '.')){
    final++;
  }
  char* tempvalue = (char*)malloc(((final)-(initial))*sizeof(char));
  memcpy(tempvalue, &s[initial], final);
  tempvalue[(final)-(initial)] = '\0';
	if(s[final] == 'r'){
		color = 1;
	}else{
		color = 0;
...
void recSerialize(char* output,link root){	//serializing recursively
	if(root == z){
		printf(".");
	...
	printf("%d%c",root->item,color);
	sprintf(output+strlen(output),"%d%c",root->item,color);
	recSerialize(output,root->l);
	recSerialize(output,root->r);
}
char* STserialize(){
	char color;
	if(head->red){
	...
	sprintf(output,"%d%c",head->item,color);

	printf("%d%c",head->item,color);

	recSerialize(output,head->l);
	recSerialize(output,head->r);
	printf("\n");
	sprintf(output +strlen(output),"\0",head->item,color); //add null character
  return output;
}
